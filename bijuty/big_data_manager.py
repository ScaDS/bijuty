...
class BigDataManager:
    ...
    def __init__(self, slurm_info: Optional[SlurmManager] = None):
        ...
        self._user_inputs: SimpleNamespace = SimpleNamespace(
            fw_name="",
            fw_home="",
            master="",
            workers=[],
    ...
    def _get_cluster_log_file(self) -> str:
        """Get the cluster log file path."""
        return f"{self._user_inputs.log_dir}/cluster_log"

    # Cluster Management
    def _wait_for_cluster_init(self, timeout: int = 30) -> bool:
    ...
    def _wait_for_cluster_stop(self, timeout: int = 30) -> bool:
    ...
    def _cleanup_cluster(self) -> None:
        ...
        if not required_procs_list or all(not p for p in required_procs_list):
            logger.error(
                f"No process names defined for {self._user_inputs.fw_name}")
            return
        found_procs = self._find_cluster_processes(required_procs_list)
        if not found_procs:
            logger.info(
                f"No running processes found for {self._user_inputs.fw_name}")
            return
        logger.info(
            f"Terminating {len(found_procs)} {self._user_inputs.fw_name} processes")
    ...
    def _verify_cluster_workers(self) -> tuple[bool, int, Exception | str]:
        ...
        worker_list = self._user_inputs.workers
        ...
        rest_api_port = self._user_inputs.rest_api_port
        host = self._user_inputs.master
        if framework == "spark":
            port = 8080
            url = f"http://{host}:{rest_api_port}/json/"
        elif framework == "flink":
            port = 8081
            url = f"http://{host}:{rest_api_port}/taskmanagers"
        else:
            return False, 0, "no workers configured"
    ...
    def initialize_user_input(self, props: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._user_inputs.workers: list = list(props.get("workers"))
            self._user_inputs.workers_count: int = len(
                list(props.get("workers")))
            ...
            self._fw_mapping: dict = props\
                .get("fw_mapping")\
                .get(str(self._user_inputs.fw_name).upper())
            self._user_inputs.rest_api_port = self._fw_mapping.rest_api_port
            self._initialized = True
        except Exception as e:
            raise Exception(
                f"Error while initializing user input for big data manager: {e}")
    ...
    def is_cluster_up(self) -> bool:
        ...
        if not self._initialized:
            logger.debug(
                "Can't check the cluster status as BigDataManager is not initialized.")
            return False
        ...
        current_user = getpass.getuser()  # This is another source of truth, get rid of it
        expected_workers = self._user_inputs.workers_count
        ...
        logger.debug(
            f"Worker Up: {workers_up}, Worker count:{worker_count}, Error Msg: {err_msg}")
    ...
    def start_cluster(self) -> bool:
        ...
        if self.is_cluster_up():
            logger.info(
                f"{self._user_inputs.fw_name} cluster already running - stopping first")
            self.stop_cluster()
        ...
        logger.debug(
            f"Start cluster command ran successfully: {result.stdout}")

        if self._wait_for_cluster_init(timeout=60):
            logger.info(
                f"{self._user_inputs.fw_name} cluster started successfully")
            return True
    ...
    def stop_cluster(self) -> bool:
        ...
        try:
            ...
            run_bash_command(
                f"nohup {full_cmd} >> {log_path} 2>&1", shell=True)

            if self._wait_for_cluster_stop(timeout=30):
                logger.info(
                    f"{self._user_inputs.fw_name} cluster stopped successfully")
                return True
            else:
                logger.warning(
