#!/bin/bash
# TODO
# - Make functions posix compliant

#Check if the flags "u" or "e" is already set
RESET_FLAG_U_REQUIRED=false
RESET_FLAG_E_REQUIRED=false
if [[ ! "$-" =~ u ]]; then
	RESET_FLAG_U_REQUIRED=true
	set -u
fi
if [[ ! "$-" =~ e ]]; then
	RESET_FLAG_E_REQUIRED=true
	set -e
fi
...
function create_configuration_dir {
	...
	if [ ! -e "$META_CONF" ]; then
		...
		if [[ "${FRAMEWORK_NAME:-not_set}" == "not_set" ]]; then
			...
			local FRAMEWORK_CPUS_PER_NODE=${SLURM_CPUS_PER_NODE}
			local FRAMEWORK_MEM_PER_NODE=${SLURM_MEM_PER_NODE}
...
EOF
			...
			#replace keywords:
			for FRAMEWORK_FILE in $(echo $FRAMEWORK_CONF_FILES); do
			...
			if [ "$FRAMEWORK_DATA_DIR" = "$FRAMEWORK_LOCAL_DIR" ]; then
				for node in $FRAMEWORK_MASTER_NODE $(cat $FRAMEWORK_CONF_DIR/$FRAMEWORK_SLAVE_FILE); do
				...
				for node in $FRAMEWORK_MASTER_NODE $(cat $FRAMEWORK_CONF_DIR/$FRAMEWORK_SLAVE_FILE); do
					...
					# post-increment does not work here, as it returns 1,
					# which, with set -e, let's everything fail
					let ++i
				done
			fi
		...
		while [ $# -gt 0 ]; do
			...
			'--framework' | '-f')
				...
				'spark' | 'hadoop' | 'flink' | 'kafka')
					...
					;;
				*)
					err_pr "Unsupported value ($2) used for parameter \"$arg\"."
					return 1
					;;
				esac
			...
			'--destination' | '-d')
				...
				;;
			-*)
				err_pr "Unsupported parameter \"$arg\" used."
				return 1
				;;
			*)
				print_usage
